...
describe('Command Validation', () => {
  describe('parseCommand', () => {
    it('splits commands by chain operators', () => {
      expect(parseCommand('npm test && npm run build')).toEqual([
        'npm test',
        'npm run build',
      ]);
      expect(parseCommand('npm test || npm run build')).toEqual([
    ...
    it('preserves quoted content', () => {
      expect(parseCommand('npm test "param with | inside"')).toEqual([
        'npm test "param with | inside"',
      ]);
    ...
    it('handles subshell patterns', () => {
      expect(parseCommand('npm test $(echo test)')).toEqual([
        'npm test',
        'echo test',
      ]);
      expect(parseCommand('npm test `echo test`')).toEqual([
  ...
  describe('isAllowedSingleCommand', () => {
    ...
    it('matches commands case-insensitively', () => {
      ...
      expect(
        isAllowedSingleCommand('npm TEST --coverage', allowedCommands)
      ).toBe(true);
    ...
    it('matches command prefixes', () => {
      ...
      expect(isAllowedSingleCommand('npm run build', allowedCommands)).toBe(
        true
      );
      expect(
        isAllowedSingleCommand('echo "hello world"', allowedCommands)
      ).toBe(true);
    });
  ...
  describe('validateCommand', () => {
    ...
    it('validates chained commands', () => {
      expect(
        validateCommand('npm test && npm run build', allowedCommands)
      ).toBe(true);
      expect(validateCommand('npm test && dangerous', allowedCommands)).toBe(
        false
      );
      expect(
        validateCommand('npm test | Select-String "Error"', allowedCommands)
      ).toBe(true);
      expect(validateCommand('npm test | rm -rf /', allowedCommands)).toBe(
        false
      );
    });
    ...
    it('handles quoted content correctly', () => {
      expect(
        validateCommand('npm test "param with | inside"', allowedCommands)
      ).toBe(true);
      expect(validateCommand('echo "hello | world"', allowedCommands)).toBe(
        true
      );
      expect(
        validateCommand('npm test "param with && inside"', allowedCommands)
    ...
    it('handles subshell execution attempts', () => {
      expect(
        validateCommand('npm test $(echo dangerous)', allowedCommands)
      ).toBe(false);
      expect(validateCommand('npm test `rm -rf /`', allowedCommands)).toBe(
        false
      );
    ...
    it('handles PowerShell patterns', () => {
      ...
      expect(
        validateCommand('npm test | Select-String | dangerous', allowedCommands)
      ).toBe(false);
    });
