...
const ChatView = ({
  ...
  const [coolclineAsk, setCoolClineAsk] = useState<CoolClineAsk | undefined>(
    undefined
  );
  ...
  const isReadOnlyToolAction = useCallback(
  ...
  const isWriteToolAction = useCallback(
    (message: CoolClineMessage | undefined) => {
      if (message?.type === 'ask') {
        if (!message.text) {
          return true;
        ...
        const tool = JSON.parse(message.text);
        return ['editedExistingFile', 'appliedDiff', 'newFileCreated'].includes(
          tool.tool
        );
      }
      return false;
  ...
  const isAutoApproved = useCallback(
    (message: CoolClineMessage | undefined) => {
      ...
      return (
        ...
        (alwaysAllowReadOnly &&
          message.ask === 'tool' &&
          isReadOnlyToolAction(message)) ||
        (alwaysAllowWrite &&
          message.ask === 'tool' &&
          isWriteToolAction(message)) ||
        (alwaysAllowExecute &&
          message.ask === 'command' &&
          isAllowedCommand(message)) ||
        (alwaysAllowMcp &&
          message.ask === 'use_mcp_server' &&
          isMcpToolAlwaysAllowed(message))
      );
    },
    [
      autoApprovalEnabled,
      ...
      {task ? (
        ...
        >
          ...
          <div style={{ padding: '0 20px', flexShrink: 0 }}>
            ...
            <p>
              Cool Cline is a proactive programming assistant that enhances your
              development workflow with advanced AI capabilities. It helps you
              write code more efficiently, optimize queries, auto-fix errors,
              run command-line tasks, and perform browser automation tests.
              <VSCodeLink
