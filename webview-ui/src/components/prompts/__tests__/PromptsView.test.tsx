...
const renderPromptsView = (props = {}) => {
  ...
  return render(
    <ExtensionStateContext.Provider
      value={{ ...mockExtensionState, ...props } as any}
    >
...
describe('PromptsView', () => {
  ...
  it('switches between tabs correctly', async () => {
    const { rerender } = render(
      <ExtensionStateContext.Provider
        value={{ ...mockExtensionState, mode: 'code' } as any}
      >
    ...
    rerender(
      <ExtensionStateContext.Provider
        value={{ ...mockExtensionState, mode: 'ask' } as any}
      >
        <PromptsView onDone={jest.fn()} />
      </ExtensionStateContext.Provider>
    );
    ...
    rerender(
      <ExtensionStateContext.Provider
        value={{ ...mockExtensionState, mode: 'architect' } as any}
      >
        <PromptsView onDone={jest.fn()} />
      </ExtensionStateContext.Provider>
    );
  ...
  it('resets role definition only for built-in modes', async () => {
    ...
    expect(
      screen.queryByTestId('role-definition-reset')
  ...
  it('handles API configuration selection', () => {
    ...
    expect(mockExtensionState.setEnhancementApiConfigId).toHaveBeenCalledWith(
      'config1'
    );
    expect(vscode.postMessage).toHaveBeenCalledWith({
