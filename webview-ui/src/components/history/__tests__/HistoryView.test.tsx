...
describe('HistoryView', () => {
  ...
  it('handles search functionality', async () => {
    ...
    const mostRelevantRadio =
      within(radioGroup).getByLabelText('Most Relevant');
    ...
    const updatedRadio = await within(radioGroup).findByRole('radio', {
      name: 'Most Relevant',
      checked: true,
    });
  ...
  it('handles sort options correctly', async () => {
    ...
    const checkedOldestRadio = await within(radioGroup).findByRole('radio', {
      name: 'Oldest',
      checked: true,
    });
    expect(checkedOldestRadio).toBeInTheDocument();

    const mostExpensiveRadio =
      within(radioGroup).getByLabelText('Most Expensive');
  ...
  it('handles task copying', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText('Prompt Copied to Clipboard')
  ...
  it('displays token counts correctly', () => {
    ...
    const tokensContainer =
      within(taskContainer).getByTestId('tokens-container');
    expect(within(tokensContainer).getByTestId('tokens-in')).toHaveTextContent(
      '100'
    );
    expect(within(tokensContainer).getByTestId('tokens-out')).toHaveTextContent(
      '50'
    );
  });
  ...
  it('displays cache information when available', () => {
    ...
    expect(
      within(cacheContainer).getByTestId('cache-writes')
    ).toHaveTextContent('+50');
    expect(within(cacheContainer).getByTestId('cache-reads')).toHaveTextContent(
      '25'
    );
  });
