...
export function isToolAllowedForMode(
  ...
  return mode.groups.some((group) =>
    TOOL_GROUPS[group].includes(tool as string)
  );
}
...
export const defaultPrompts: Readonly<CustomPrompts> = Object.freeze(
  Object.fromEntries(
    modes.map((mode) => [mode.slug, { roleDefinition: mode.roleDefinition }])
  )
);
