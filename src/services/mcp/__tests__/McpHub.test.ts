...
describe('McpHub', () => {
  ...
  beforeEach(() => {
    ...
    mockProvider = {
      ensureSettingsDirectoryExists: jest
        .fn()
        .mockResolvedValue('/mock/settings/path'),
      ensureMcpServersDirectoryExists: jest
        .fn()
        .mockResolvedValue('/mock/settings/path'),
  ...
  describe('toggleToolAlwaysAllow', () => {
    it('should add tool to always allow list when enabling', async () => {
      ...
      expect(writtenConfig.mcpServers['test-server'].alwaysAllow).toContain(
        'new-tool'
      );
    });
    ...
    it('should remove tool from always allow list when disabling', async () => {
      ...
      const writeCall = (fs.writeFile as jest.Mock).mock.calls[0];
      const writtenConfig = JSON.parse(writeCall[1]);
      expect(writtenConfig.mcpServers['test-server'].alwaysAllow).not.toContain(
        'existing-tool'
      );
    });
  ...
  describe('callTool', () => {
    ...
    it('should throw error if server not found', async () => {
      ...
      ).rejects.toThrow('No connection found for server: non-existent-server');
    });
  });
});
