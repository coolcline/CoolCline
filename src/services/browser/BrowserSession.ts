...
export class BrowserSession {
  ...
  async launchBrowser(): Promise<void> {
    ...
    this.browser = await stats.puppeteer.launch({
      ...
      defaultViewport: (() => {
        const size =
          (this.context.globalState.get('browserViewportSize') as
            | string
            | undefined) || '900x600';
  ...
  async doAction(
    ...
    let screenshotBase64 = await this.page.screenshot({
      ...
      type: 'webp',
      quality:
        ((await this.context.globalState.get('screenshotQuality')) as
          | number
          | undefined) ?? 75,
    });
  ...
  async scrollDown(): Promise<BrowserActionResult> {
    const size =
      ((await this.context.globalState.get('browserViewportSize')) as
        | string
        | undefined) || '900x600';
