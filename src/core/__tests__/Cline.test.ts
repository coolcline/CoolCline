...
jest.mock('strip-ansi', () => ({
  ...
  default: jest
    .fn()
    .mockImplementation((str) => str.replace(/\u001B\[\d+m/g, '')),
}));
...
describe('CoolCline', () => {
  ...
  beforeEach(() => {
    ...
    mockProvider = new CoolClineProvider(
      mockExtensionContext,
      mockOutputChannel
    ) as jest.Mocked<CoolClineProvider>;
  ...
  describe('constructor', () => {
    ...
    it('should use provided fuzzy match threshold', () => {
      const getDiffStrategySpy = jest.spyOn(
        require('../diff/DiffStrategy'),
        'getDiffStrategy'
      );
  ...
  describe('getEnvironmentDetails', () => {
    ...
    beforeEach(() => {
      ...
      MockDateTimeFormat.supportedLocalesOf = jest
        .fn()
    ...
    describe('API conversation handling', () => {
      it('should clean conversation history before sending to API', async () => {
        ...
        jest
          .spyOn(coolcline.api, 'createMessage')
          .mockImplementation(cleanMessageSpy);

        // Mock getEnvironmentDetails to return empty details
        jest
          .spyOn(coolcline as any, 'getEnvironmentDetails')
          .mockResolvedValue('');

        // Mock loadContext to return unmodified content
        jest
          .spyOn(coolcline as any, 'loadContext')
        ...
        await coolcline.recursivelyMakeCoolClineRequests(
      ...
      it('should handle image blocks based on model capabilities', async () => {
        ...
        jest
          .spyOn(coolclineWithImages as any, 'getEnvironmentDetails')
          .mockResolvedValue('');
        jest
          .spyOn(coolclineWithoutImages as any, 'getEnvironmentDetails')
          .mockResolvedValue('');
        jest
          .spyOn(coolclineWithImages as any, 'loadContext')
          .mockImplementation(async (content) => [content, '']);
        jest
          .spyOn(coolclineWithoutImages as any, 'loadContext')
          .mockImplementation(async (content) => [content, '']);
        // Set up mock streams
        ...
        jest
          .spyOn(coolclineWithImages.api, 'createMessage')
          .mockImplementation(imagesSpy);
        jest
          .spyOn(coolclineWithoutImages.api, 'createMessage')
          .mockImplementation(noImagesSpy);
      ...
      describe('loadContext', () => {
        it('should process mentions in task and feedback tags', async () => {
          ...
          const mockParseMentions = jest
            .fn()
            .mockImplementation((text) => `processed: ${text}`);
          jest
            .spyOn(require('../../core/mentions'), 'parseMentions')
            .mockImplementation(mockParseMentions);
