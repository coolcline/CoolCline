...
export function applyContextMatching(
  ...
  for (const change of hunk.changes) {
    if (change.type === 'context') {
      ...
      if (sourceIndex < content.length) {
        ...
        const line = change.indent
          ? change.indent + change.content
          : change.content;
        newResult.push(line);
      }
      sourceIndex++;
  ...
  const afterText = newResult
    .slice(matchPosition, newResult.length - (content.length - sourceIndex))
    .join('\n');
...
export function applyDMP(
  ...
  const targetText = content
    .slice(matchPosition, matchPosition + beforeLineCount)
    .join('\n');
...
export async function applyGitFallback(
  hunk: Hunk,
  content: string[]
): Promise<EditResult> {
  ...
  try {
    ...
    try {
      ...
      console.log(
        'Strategy 1 - Attempting checkout of:',
        originalCommit.commit
      );
      await git.raw(['checkout', originalCommit.commit]);
      try {
        console.log(
          'Strategy 1 - Attempting cherry-pick of:',
...
export async function applyEdit(
  ...
  const strategies = [
    ...
    {
      name: 'context',
      apply: () => applyContextMatching(hunk, content, matchPosition),
    },
