...
function evaluateContentUniqueness(
  searchStr: string,
  content: string[]
): number {
...
export function validateEditResult(hunk: Hunk, result: string): number {
  ...
  const expectedText = hunk.changes
    ...
    .map((change) =>
      change.indent ? change.indent + change.content : change.content
    )
    .join('\n');
...
function validateContextLines(
  ...
  const uniquenessScore = evaluateContentUniqueness(
    searchStr,
    content.split('\n')
  );
  ...
  return similarity < threshold
    ? similarity * 0.3 + uniquenessBoost
    : similarity + uniquenessBoost;
}
...
function createOverlappingWindows(
  ...
  const effectiveWindowSize = Math.max(
    searchSize,
    Math.min(searchSize * 2, MAX_WINDOW_SIZE)
  );
...
function combineOverlappingMatches(
  ...
  for (const match of matches) {
    ...
    if (overlapping.length > 0) {
      ...
      const avgConfidence =
        (match.confidence +
          overlapping.reduce((sum, m) => sum + m.confidence, 0)) /
...
export function findExactMatch(
  ...
  const searchLines = searchStr.split('\n');
  const windows = createOverlappingWindows(
  ...
  windows.forEach((windowData, windowIndex) => {
    ...
    if (exactMatch !== -1) {
      ...
      const contextSimilarity = validateContextLines(
        searchStr,
        matchedContent,
        confidenceThreshold
      );
  ...
  return combinedMatches.length > 0
    ? combinedMatches[0]
    : { index: -1, confidence: 0, strategy: 'exact' };
}
...
function identifyAnchors(searchStr: string): {
  first: string | null;
  last: string | null;
} {
  const searchLines = searchStr.split('\n');
...
export function findAnchorMatch(
  ...
  const expectedContext = searchLines
    .slice(searchLines.indexOf(first) + 1, searchLines.indexOf(last))
    .join('\n');
  ...
  if (
    contextSimilarity <
    getAdaptiveThreshold(content.length, confidenceThreshold)
  ) {
    return { index: -1, confidence: 0, strategy: 'anchor' };
  }
