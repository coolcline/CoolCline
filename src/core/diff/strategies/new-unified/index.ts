  ...
  async applyDiff(
    ...
    if (!parsedDiff.hunks.length) {
      return {
        success: false,
        error:
          'No hunks found in diff. Please ensure your diff includes actual changes and follows the unified diff format.',
      };
    }
    ...
    for (const hunk of parsedDiff.hunks) {
      ...
      if (confidence < this.confidenceThreshold) {
        ...
        for (const subHunk of subHunks) {
          ...
          const subSearchResult = findBestMatch(
            subContextStr,
            subHunkResult,
            0,
        ...
        const contextLines = hunk.changes.filter(
          (c) => c.type === 'context'
        ).length;
      ...
      if (editResult.confidence >= this.confidenceThreshold) {
        ...
        errorMsg += 'Debug Info:\n';
        errorMsg +=
          "- The location was found but the content didn't match exactly\n";
        errorMsg +=
          '- This usually means the file has been modified since the diff was created\n';
        errorMsg +=
          '- Or the diff may be targeting a different version of the file\n';
        errorMsg += '\nPossible Solutions:\n';
        ...
        errorMsg +=
          '3. Ensure your diff targets the correct version of the file';
