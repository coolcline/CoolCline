      ...
      async (message: WebviewMessage) => {
        switch (message.type) {
          case 'webviewDidLaunch':
            ...
            this.configManager
              ...
              .then(async (listApiConfig) => {
                ...
                let currentConfigName = (await this.getGlobalState(
                  'currentApiConfigName'
                )) as string;

                if (currentConfigName) {
                  if (
                    !(await this.configManager.hasConfig(currentConfigName))
                  ) {
          ...
          case 'alwaysAllowMcp':
            ...
            break;
          case 'askResponse':
            this.coolcline?.handleWebviewAskResponse(
              message.askResponse!,
              message.text,
              message.images
            );
          ...
          case 'openFile':
            openFile(
              message.text!,
              message.values as { create?: boolean; content?: string }
            );
            break;
          case 'openMention':
            ...
            if (this.coolcline) {
              ...
              await pWaitFor(
                () =>
                  ...
                  this.coolcline.didFinishAborting,
                {
                  timeout: 3_000,
                }
              ).catch(() => {
          ...
          case 'toggleToolAlwaysAllow':
            try {
              ...
              console.error(
                `Failed to toggle auto-approve for tool ${message.toolName}:`,
                error
              );
            }
            break;
          ...
          case 'toggleMcpServer':
            try {
              ...
              console.error(
                `Failed to toggle MCP server ${message.serverName}:`,
                error
              );
            }
            break;
          ...
          case 'mode':
            ...
            const savedConfigId =
              await this.configManager.getModeConfigId(newMode);
            ...
            if (savedConfigId) {
              ...
              const currentApiConfigName = await this.getGlobalState(
                'currentApiConfigName'
              );
          ...
          case 'updateEnhancedPrompt':
            const existingPrompts =
              (await this.getGlobalState('customPrompts')) || {};
          ...
          case 'deleteMessage': {
            ...
            if (
              (answer === 'Just this message' ||
                answer === 'This and all subsequent messages') &&
              this.coolcline &&
              ...
              const apiConversationHistoryIndex =
                this.coolcline.apiConversationHistory.findIndex(
                  (msg) => msg.ts && msg.ts >= timeCutoff
                );
              ...
              if (messageIndex !== -1) {
                ...
                if (answer === 'Just this message') {
                  ...
                  const nextUserMessage = this.coolcline.coolclineMessages
                    ...
                    .find(
                      (msg) => msg.type === 'say' && msg.say === 'user_feedback'
                    );
            ...
            if (message.text) {
              try {
                ...
                const getEnhancePrompt = (
                  value: string | PromptComponent | undefined
                ): string => {
          ...
          case 'getSystemPrompt':
            try {
              ...
              const systemPrompt = await SYSTEM_PROMPT(
                this.context,
                cwd,
                apiConfiguration.openRouterModelInfo?.supportsComputerUse ??
                  false,
          ...
          case 'deleteApiConfiguration':
            if (message.text) {
              ...
              try {
                ...
                let currentApiConfigName = await this.getGlobalState(
                  'currentApiConfigName'
                );
                if (
                  message.text === currentApiConfigName &&
                  listApiConfig?.[0]?.name
                ) {
                  const apiConfig = await this.configManager.loadConfig(
                    listApiConfig[0].name
          ...
          case 'getListApiConfiguration':
            try {
              ...
              vscode.window.showErrorMessage(
                'Failed to get list api configuration'
              );
            }
            break;
          case 'experimentalDiffStrategy':
            ...
            if (this.coolcline) {
              await this.coolcline.updateDiffStrategy(message.bool ?? false);
            }
            await this.postStateToWebview();
            break;
      ...
      if (response.data) {
        ...
        for (const rawModel of rawModels) {
          const modelInfo: ModelInfo = {
            ...
            supportsComputerUse:
              rawModel.capabilities?.includes('computer_use'),
  ...
  async getState() {
    const [
      ...
      this.getGlobalState('enhancementApiConfigId') as Promise<
        string | undefined
      >,
      this.getGlobalState('experimentalDiffStrategy') as Promise<
        boolean | undefined
      >,
