...
jest.mock('../../diff/DiffStrategy', () => ({
  getDiffStrategy: jest.fn().mockImplementation(() => ({
    getToolDescription: jest
      .fn()
      .mockReturnValue('apply_diff tool description'),
  })),
}));
...
jest.mock('../../prompts/system', () => ({
  SYSTEM_PROMPT: jest
    .fn()
    .mockImplementation(async () => 'mocked system prompt'),
  codeMode: 'code',
}));
...
describe('CoolClineProvider', () => {
  ...
  test('loads saved API config when switching modes', async () => {
    ...
    expect(provider.configManager.getModeConfigId).toHaveBeenCalledWith(
      'architect'
    );
    expect(provider.configManager.loadConfig).toHaveBeenCalledWith(
      'test-config'
    );
    expect(mockContext.globalState.update).toHaveBeenCalledWith(
  ...
  test('saves current config when switching to mode without config', async () => {
    ...
    expect(provider.configManager.setModeConfig).toHaveBeenCalledWith(
      'architect',
      'current-id'
    );
  });
  ...
  test('saves config as default for current mode when loading config', async () => {
    ...
    provider.configManager = {
      loadConfig: jest
        .fn()
        .mockResolvedValue({ apiProvider: 'anthropic', id: 'new-id' }),
      listConfig: jest
        .fn()
        .mockResolvedValue([
  ...
  test('file content includes line numbers', async () => {
    const {
      extractTextFromFile,
    } = require('../../../integrations/misc/extract-text');
  ...
  describe('deleteMessage', () => {
    ...
    test('handles "Just this message" deletion correctly', async () => {
      ...
      expect(
        mockCoolCline.overwriteApiConversationHistory
    ...
    test('handles "This and all subsequent messages" deletion correctly', async () => {
      ...
      expect(
        mockCoolCline.overwriteApiConversationHistory
      ).toHaveBeenCalledWith([mockApiHistory[0]]);
    });
  ...
  describe('getSystemPrompt', () => {
    ...
    test('handles errors gracefully', async () => {
      ...
      jest
        .spyOn(systemPrompt, 'SYSTEM_PROMPT')
    ...
    test('uses code mode custom instructions', async () => {
      ...
      const mockAddCustomInstructions = (
        jest.requireMock('../../prompts/sections/custom-instructions') as any
      ...
      jest
        .spyOn(systemPromptModule, 'SYSTEM_PROMPT')
        .mockImplementation(async () => {
          await mockAddCustomInstructions(
