...
export function getSystemInfoSection(
  ...
  const findModeBySlug = (slug: string, modes?: ModeConfig[]) =>
    modes?.find((m) => m.slug === slug);

  const currentModeName =
    findModeBySlug(currentMode, customModes)?.name || currentMode;
  const codeModeName =
    findModeBySlug(defaultModeSlug, customModes)?.name || 'Code';
