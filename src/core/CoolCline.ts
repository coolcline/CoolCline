...
import {
  CoolClineProvider,
  GlobalFileNames,
} from './webview/CoolClineProvider';
...
export class CoolCline {
  ...
  private async saveCoolClineMessages() {
    try {
      ...
      const apiMetrics = getApiMetrics(
        combineApiRequests(
          combineCommandSequences(this.coolclineMessages.slice(1))
        )
      );
  ...
  private async resumeTaskFromHistory() {
    ...
    if (lastApiReqStartedIndex !== -1) {
      const lastApiReqStarted =
        modifiedCoolClineMessages[lastApiReqStartedIndex];
      const { cost, cancelReason }: CoolClineApiReqInfo = JSON.parse(
    ...
    const wasRecent =
      lastCoolClineMessage?.ts && Date.now() - lastCoolClineMessage.ts < 30_000;
  ...
  private async initiateTaskLoop(userContent: UserContent): Promise<void> {
    ...
    while (!this.abort) {
      const didEndLoop = await this.recursivelyMakeCoolClineRequests(
        nextUserContent,
        includeFileDetails
      );
  ...
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    if (mcpEnabled ?? true) {
      ...
      await pWaitFor(() => mcpHub!.isConnecting !== true, {
        timeout: 10_000,
      }).catch(() => {
    ...
    if (previousApiReqIndex >= 0) {
      ...
      if (previousRequest && previousRequest.text) {
        const {
          ...
          cacheWrites,
          cacheReads,
        }: CoolClineApiReqInfo = JSON.parse(previousRequest.text);
        const totalTokens =
          (tokensIn || 0) +
          (tokensOut || 0) +
          (cacheWrites || 0) +
      ...
      case 'tool_use':
        ...
        switch (block.name) {
          case 'write_to_file': {
            ...
            const predictedLineCount: number | undefined = parseInt(
              block.params.line_count ?? '0'
            );
                ...
                if (
                  ...
                  if (this.diffStrategy) {
                    ...
                    vscode.window
                      ...
                      .then((selection) => {
                        if (
                          selection === 'Follow this guide to fix the issue'
                        ) {
            ...
            try {
              if (block.partial) {
                ...
                if (!diffResult.success) {
                  this.consecutiveMistakeCount++;
                  const currentCount =
                    (this.consecutiveMistakeCountForApplyDiff.get(relPath) ||
                      0) + 1;
                  this.consecutiveMistakeCountForApplyDiff.set(
                    relPath,
                    currentCount
                  );
          ...
          case 'list_code_definition_names': {
            ...
            try {
              if (block.partial) {
                ...
                const absolutePath = path.resolve(cwd, relDirPath);
                const result =
                  await parseSourceCodeForDefinitionsTopLevel(absolutePath);
                const completeMessage = JSON.stringify({
                  ...sharedMessageProps,
                  content: result,
                } satisfies CoolClineSayTool);
            ...
            try {
              if (block.partial) {
                ...
                if (action === 'launch') {
                  ...
                  switch (action) {
                    case 'click':
                      browserActionResult = await this.browserSession.click(
                        coordinate!
                      );
                      break;
                    case 'type':
                      browserActionResult = await this.browserSession.type(
                        text!
                      );
                      break;
                    case 'scroll_down':
  ...
  async recursivelyMakeCoolClineRequests(
    ...
    try {
        ...
        if (!this.abandoned) {
          ...
          if (history) {
            await this.providerRef
              .deref()
              ?.initCoolClineWithHistoryItem(history.historyItem);
            // await this.providerRef.deref()?.postStateToWebview()
          }
        }
      }
      ...
      if (assistantMessage.length > 0) {
        ...
        const recDidEndLoop = await this.recursivelyMakeCoolClineRequests(
          this.userMessageContent
        );
        ...
        await this.addToApiConversationHistory({
          role: 'assistant',
          content: [
            { type: 'text', text: 'Failure: I did not provide a response.' },
          ],
        });
      }
  ...
  async getEnvironmentDetails(includeFileDetails: boolean = false) {
    ...
    if (
      ...
      const currentModeName =
        getModeBySlug(currentMode, customModes)?.name ?? currentMode;
      const defaultModeName =
        getModeBySlug(defaultModeSlug, customModes)?.name ?? defaultModeSlug;
