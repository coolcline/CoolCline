...
describe('CustomModesManager', () => {
  ...
  describe('File Operations', () => {
    test("creates settings directory if it doesn't exist", async () => {
      const configPath = path.join(
        mockStoragePath,
        'settings',
        'coolcline_custom_modes.json'
      );
      await manager.getCustomModesFilePath();

      expect(fs.mkdir).toHaveBeenCalledWith(path.dirname(configPath), {
        recursive: true,
      });
    });
    ...
    test('watches file for changes', async () => {
      ...
      const registerCall = (vscode.workspace.onDidSaveTextDocument as jest.Mock)
        .mock.calls[0];
  ...
  describe('Mode Operations', () => {
    ...
    test('deletes custom mode', async () => {
      ...
      expect(fs.writeFile).toHaveBeenCalledWith(
        expect.any(String),
        expect.not.stringContaining(validMode.name)
      );
      expect(mockOnUpdate).toHaveBeenCalled();
    });
    ...
    test('queues write operations', async () => {
      ...
      (fs.readFile as jest.Mock).mockImplementation(() =>
        JSON.stringify({ customModes: currentModes })
      );
      ...
      expect(finalContent.customModes.map((m: ModeConfig) => m.name)).toContain(
        'Mode 1'
      );
      expect(finalContent.customModes.map((m: ModeConfig) => m.name)).toContain(
        'Mode 2'
