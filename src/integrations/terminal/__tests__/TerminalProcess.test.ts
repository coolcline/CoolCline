...
describe('TerminalProcess', () => {
  ...
  describe('run', () => {
    ...
    it('handles terminals without shell integration', async () => {
      ...
      expect(noShellTerminal.sendText).toHaveBeenCalledWith(
        'test command',
        true
      );
    });
  ...
  describe('removeLastLineArtifacts', () => {
    it('removes terminal artifacts from output', () => {
      ...
      for (const [input, expected] of cases) {
        expect(terminalProcess['removeLastLineArtifacts'](input)).toBe(
          expected
        );
      }
    });
  ...
  describe('getUnretrievedOutput', () => {
    it('returns and clears unretrieved output', () => {
      ...
      expect(terminalProcess['lastRetrievedIndex']).toBe(
        terminalProcess['fullOutput'].length
      );
    });
  });
