...
describe('WorkspaceTracker', () => {
  ...
  it('should initialize with workspace files', async () => {
    const mockFiles = [
      ['/test/workspace/file1.ts', '/test/workspace/file2.ts'],
      false,
    ];
    ...
    expect(
      (mockProvider.postMessageToWebview as jest.Mock).mock.calls[0][0]
        .filePaths
    ).toHaveLength(2);
  });
  ...
  it('should handle directory paths correctly', async () => {
    ...
    const lastCall = (
      mockProvider.postMessageToWebview as jest.Mock
