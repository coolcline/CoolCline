...
suite('CoolCline Extension Test Suite', () => {
  vscode.window.showInformationMessage('Starting CoolCline extension tests.');
    ...
    try {
      ...
      provider.postMessageToWebview = async (message) => {
        if (message.type === 'state') {
          ...
          if (message.state?.coolclineMessages?.length > 0) {
            messagesReceived = true;
            console.log('Messages in state:', message.state.coolclineMessages);
          }
        }
      ...
      while (Date.now() - startTime < timeout) {
        // Check provider.coolclineMessages
        const messages = provider.coolclineMessages;
        if (messages && messages.length > 0) {
          ...
          const hasResponse = messages.some(
            (m: { type: string; text: string }) =>
              m.type === 'say' &&
              m.text &&
              m.text.toLowerCase().includes('coolcline')
          );
          if (hasResponse) {
            console.log(
              'Found response containing "CoolCline" in provider messages'
            );
        ...
        // Check provider.coolcline.coolclineMessages
        const coolclineMessages = provider.coolcline?.coolclineMessages;
        if (coolclineMessages && coolclineMessages.length > 0) {
          console.log(
            'CoolCline messages:',
            JSON.stringify(coolclineMessages, null, 2)
          );
          // @ts-ignore
          const hasResponse = coolclineMessages.some(
            (m: { type: string; text: string }) =>
              m.type === 'say' &&
              m.text &&
              m.text.toLowerCase().includes('coolcline')
          ...
          if (hasResponse) {
            console.log(
              'Found response containing "CoolCline" in coolcline messages'
            );
            responseReceived = true;
            break;
          }
      ...
      if (!responseReceived) {
        ...
        console.log(
          'Final coolcline messages:',
          provider.coolcline?.coolclineMessages
        );
        throw new Error(
          'Did not receive expected response containing "CoolCline"'
        );
      }
    } finally {
      panel.dispose();
